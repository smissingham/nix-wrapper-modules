...
in
{
  ...
  config = {
    ...
    meta.description = ''
      ...
      Example 1 (Simple):
      Wrap the sops CLI with nix-owned creation rules:
        wrappers.sops.wrap {
          inherit pkgs;
      ...
      Example 2 (Advanced):
      Or sops-wrap another wrapper, injecting decrypted secrets at launch.
      Decryption is best-effort: on failure, program still launches.
